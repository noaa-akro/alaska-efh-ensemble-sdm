...
raster.stack <- terra::rast(paste0("M:/HCD/shared_hcd_projects/2028 EFH SDM/2028_Covariates/", region, "/covariate_raster_stack_", region, ".tiff"))

names(raster.stack) <- c("lon", "lat", "bdepth", "btemp", "sponge", "coral", "pen", "phi", "aspectE", "aspectN",  "bcurrentU", "bcurrentUSD", "bcurrentV", "bcurrentVSD", "tmax", "BPI", "curve", "rocky", "slope")
...
#if we want random folds
random.folds <- rep(LETTERS[1:10], length.out = nrow(species.data))
species.data$random_folds <- sample(random.folds, size = nrow(species.data), replace = F)

#if we want spatial folds using blockCV
# ===================================================================
# --- SPATIAL BLOCK FOLD ASSIGNMENT (blockCV) ---
# ===================================================================

# 1. Convert species.data to sf spatial object using EPSG:3338
data_sf <- sf::st_as_sf(species.data, coords = c("lon", "lat"), crs = 3338)

# 2. Run blockCV to generate 50 km spatial blocks
sb <- blockCV::cv_spatial(
  x = data_sf,
  k = 10,                  # Number of folds
  size = 50000,           # 50,000 meters = 50 km
  hexagon = FALSE,
  selection = "random",
  iteration = 100,
  progress = FALSE
...
# 3. Extract block polygons and assign fold IDs positionally (prevents duplicate rows)
blocks_poly <- sb$blocks
block_intersects <- sf::st_intersects(data_sf, blocks_poly)

# Assign fold IDs to a new/overwritten 'Folds' column
species.data$Folds <- sapply(block_intersects, function(x) {
  if (length(x) > 0) blocks_poly$folds[x[1]] else NA
})

# 4. Assign boundary points (if any fell slightly outside blocks) to nearest fold
if (any(is.na(species.data$Folds))) {
  na_idx <- which(is.na(species.data$Folds))
  nearest_idx <- sf::st_nearest_feature(data_sf[na_idx, ], blocks_poly)
  species.data$Folds[na_idx] <- blocks_poly$folds[nearest_idx]
}

# Quick check on fold counts
table(species.data$Folds)
...
# gam.form <- stats::formula(paste0(species, " ~ s(lon, lat, bs = 'ds', m = c(1,0.5), k= 10) + s(bdepth, bs='tp',m=1,k=4) +
#                     s(btemp, bs='tp', m=1, k=4) + s(slope, bs = 'tp', m = 1,k = 4) + as.factor(sponge) + offset(logarea)"))

gam.form <- stats::formula(paste0(species, " ~ s(lon, lat, bs = 'ds', m = c(1,.5), k=10) + 
                                  s(bdepth, bs='tp',m=1,k=4) + 
                                  s(btemp, bs='tp',m=1,k=4) +
                                  s(slope, bs='tp',m=1,k=4) + 
                                  s(BPI, bs='tp',m=1,k=4) + 
                                  s(curve, bs='tp',m=1,k=4) + 
                                  s(rocky, bs='tp',m=1,k=4) +
                                  s(aspectE, bs='tp',m=1,k=4) + 
                                  s(aspectN, bs='tp',m=1,k=4) +
                                  s(bcurrentU, bcurrentV, bs = 'ds', m = c(1,.5), k=10) +
                                  s(bcurrentUSD, bcurrentVSD, bs = 'ds', m = c(1,.5), k=10) +
                                  s(tmax, bs='tp',m=1,k=4) +
                                  s(phi, bs='tp',m=1,k=4) +
                                  as.factor(sponge) + 
                                  as.factor(coral) + 
                                  as.factor(pen) + 
...
maxnet.model <- FitMaxnet(
  ...
  vars = c("lon", "lat", "btemp", "bdepth", "slope", "BPI", "curve", "rocky", "aspectE", "aspectN", 
                         "bcurrentU", "bcurrentUSD", "bcurrentV", "bcurrentVSD", "tmax", "phi"), facs = c("sponge", "coral", "pen"),
  regmult = 1, reduce = T
)
...
maxnet.effects <- GetMaxnetEffects(
  ...
  maxnet2d = list(c("lon", "lat"), c("bcurrentU","bcurrentV"),c("bcurrentUSD","bcurrentVSD")), scale.factor = maxnet.scale
)
```
...
cloglog.cv1 <- CrossValidateModel(
  model = cloglog.model, 
  model.type = "cloglog", 
  data = species.data, 
  scale.preds = T,
...
cloglog.cv2 <- CrossValidateModel(
  model = cloglog.model, 
  model.type = "cloglog", 
  data = species.data, 
  scale.preds = T,
...
#CV random
cloglog.errors <- cloglog.cv1[[1]]
cloglog.cv.models <- cloglog.cv1[[2]]
...
cloglog.rmse1 <- RMSE(pred = cloglog.errors$cvpred, obs = cloglog.errors$abund)

#BlockCV
cloglog.errors2 <- cloglog.cv2[[1]]
cloglog.cv.models2 <- cloglog.cv2[[2]]

cloglog.effects2 <- GetGAMEffects(
  model = cloglog.model, data = species.data, vars = "all", scale = "log",
  cv.model.list = cloglog.cv.models2, scale.factor = cloglog.scale
)

cloglog.rmse2 <- RMSE(pred = cloglog.errors2$cvpred, obs = cloglog.errors2$abund)

cloglog.rmse1
cloglog.rmse2

#compare the effects plots

# Function to bind effect data from two models for a specific covariate
compare_effect_curves <- function(effects1, effects2, cov_name, model1_label = "Model 1", model2_label = "Model 2") {
  
  df1 <- effects1[[cov_name]] %>% mutate(Model = model1_label)
  df2 <- effects2[[cov_name]] %>% mutate(Model = model2_label)
  
  combined_df <- bind_rows(df1, df2)
  
  # Plot the overall partial effect curve comparison
  p <- ggplot(combined_df, aes(x = x, y = effect, color = Model, fill = Model, linetype = Model)) +
    geom_line(linewidth = 1) +
    labs(
      title = paste("Partial Effect Comparison:", cov_name),
      x = cov_name,
      y = "Effect"
    ) +
    theme_minimal()
  
  # Include confidence intervals if upper/lower bounds are present
  if (all(c("upper", "lower") %in% names(combined_df))) {
...
# Example usage for 'bdepth':
compare_effect_curves(cloglog.effects, cloglog.effects2, "bdepth", "Baseline GAM", "Spatial CV GAM")



#continue for normal line
cloglog.cv <- cloglog.cv2
#CV random
...
#                     s(btemp, bs='tp',m=1,k=4) + s(slope, bs='tp',m=1,k=4) + as.factor(sponge) + offset(logarea)")
prob.form <- stats::formula(" ~ s(lon, lat, bs = 'ds', m = c(1,.5), k=10) + 
                                  s(bdepth, bs='tp',m=1,k=4) + 
                                  s(btemp, bs='tp',m=1,k=4) +
                                  s(slope, bs='tp',m=1,k=4) + 
                                  s(BPI, bs='tp',m=1,k=4) + 
...
dev.dat <- rbind(
  c(maxnet.devs, coral = 0),
  c(cloglog.devs, slope = 0, curve = 0, bcurrentU  = 0, bcurrentUSD  = 0, bcurrentV  = 0, bcurrentVSD = 0, phi = 0, sponge =0), # slope was dropped from the cloglog model, so it explains 0
  hpoisson.devs,
  poisson.devs
)
...
ensemble.breaks <- FindEFHbreaks(
  ...
  data = NULL, quantiles = c(.05, .25, .5, .75)
)
...
MakeAKGFEFHplot(region = "goa", efh.map = ensemble.efh, title.name = figure.name, , legend.title = "Percentiles")
...
gridExtra::grid.arrange(grobs = ensemble.plots, ncol = 3)
