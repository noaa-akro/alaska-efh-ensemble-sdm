...
####MEY (7/27/26) question : Do we want the covariate phi in GOA? It currently is not in the 'region_data_goa' file, so I kept it out for now, but if we want it we will need to add it to the region data
###Also I dont think tmax is added? need to look into that next 
...
raster.stack <- c(lon, lat, bathy, btemp, sponge, coral, pen, phi, aspectE, aspectN, 
                  bcurrentU, bcurrentUSD, bcurrentV, bcurrentVSD, BPI, curve, rockiness, slope)
names(raster.stack) <- c("lon", "lat", "bdepth", "btemp", "sponge", "coral", "pen", "phi", "aspectE", "aspectN", 
                         "bcurrentU", "bcurrentUSD", "bcurrentV", "bcurrentVSD", "BPI", "curve", "rocky", "slope")
...
nice.names <- data.frame(
  ...
  var = c(
    ...
    "bcurrentU*bcurrentV", "bcurrentUSD*bcurrentVSD", "aspectE*aspectN", "rockiness"
  ),
  name = c(
...
maxnet.model <- FitMaxnet(
  ...
  vars = c("lon", "lat", "btemp", "bdepth", "slope", "BPI", "curve", "rocky", "aspectE", "aspectN", 
                         "bcurrentU", "bcurrentUSD", "bcurrentV", "bcurrentVSD"), facs = c("sponge", "coral", "pen"),
  regmult = 1, reduce = T
)
